...
CUDA_VISIBLE_DEVICES="$GPU" accelerate launch --num_processes 1 train_osediff_focus_fusion.py \
 ...
 --gradient_accumulation_steps "$GRADIENT_ACCUMULATION_STEPS" --max_train_steps "$MAX_STEPS" --checkpointing_steps 250 --validation_steps 250 \
 --mixed_precision "$MIXED_PRECISION" "${NATIVE_ARGS[@]}" "${MAX_PIXEL_ARGS[@]}"
