...
if [[ "${INPUT_MODE}" == "single" ]]; then
  ...
  CONDITION_ABLATION="${CONDITION_ABLATION:-normal}"
else
  RUN_ALL_ABLATIONS="${RUN_ALL_ABLATIONS:-1}"
  CONDITION_ABLATION="${CONDITION_ABLATION:-normal}"
fi
...
ABLATION_ARGS=()
if [[ "${INPUT_MODE}" == "single" ]]; then
  ABLATION_ARGS+=(--condition_ablation normal)
elif [[ "${RUN_ALL_ABLATIONS}" == "1" ]]; then
  ABLATION_ARGS+=(--run_all_ablations)
else
  ABLATION_ARGS+=(--condition_ablation "${CONDITION_ABLATION}")
fi
...
CUDA_VISIBLE_DEVICES="$GPU" python test_osediff_focus_fusion.py --pretrained_model_name_or_path "$PRETRAINED_MODEL" --checkpoint_path "$CHECKPOINT" \
 ...
 --seed "${SEED:-123}" "${ABLATION_ARGS[@]}" "${KEEP_ARGS[@]}" "${NATIVE_ARGS[@]}" "${MAX_PIXEL_ARGS[@]}"
