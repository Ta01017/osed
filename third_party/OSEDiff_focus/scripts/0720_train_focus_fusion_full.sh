...
CUDA_VISIBLE_DEVICES="$GPUS" accelerate launch --num_processes "$NUM_PROCESSES" train_osediff_focus_fusion.py \
 ...
 --input_mode "$INPUT_MODE" --prompt_mode fixed --use_vsd "${USE_VSD:-0}" "${TRAIN_VAE_ARGS[@]}" --train_batch_size "$TRAIN_BATCH_SIZE" \
 --gradient_accumulation_steps "$GRADIENT_ACCUMULATION_STEPS" --max_train_steps "$MAX_STEPS" --checkpointing_steps "${CHECKPOINT_STEPS:-500}" \
 --validation_steps "${VALIDATION_STEPS:-500}" --mixed_precision "$MIXED_PRECISION" "${NATIVE_ARGS[@]}" "${MAX_PIXEL_ARGS[@]}"
